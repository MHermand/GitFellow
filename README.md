...
- **Enable “Sign in with GitHub”** — create an OAuth App on [github.com/settings/developers](https://github.com/settings/developers) with *Enable Device Flow* ticked (the callback URL is unused), then put its client ID in `src/lib/github-app.ts` (or pass `GITFELLOW_GITHUB_CLIENT_ID` when launching, to try it without touching the file). The ID is public; there is no secret in the device flow. The app asks for the `repo` scope, the only one that reads private repositories. Without it, the assistant only offers the token.
