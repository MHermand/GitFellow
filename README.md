...
- **Try it without a GitHub account** — `GITFELLOW_FAKE_GITHUB=1 npm run dev` replaces GitHub with three repositories and eight weeks of made-up commits. The screenshots above come from it: `npm i -D playwright && npx playwright install chromium` once, then `node scripts/pack.mjs && node scripts/screenshots.mjs`.
- **Enable “Sign in with GitHub”** — create an OAuth App on [github.com/settings/developers](https://github.com/settings/developers) with *Enable Device Flow* ticked and *Expire user access tokens* **unticked** (refreshing an expired token needs a client secret, which an application installed on people's machines cannot hold; the callback URL is unused), then put its client ID in `src/lib/github-app.ts` (or pass `GITFELLOW_GITHUB_CLIENT_ID` when launching, to try it without touching the file). The ID is public; there is no secret in the device flow. The app asks for the `repo` scope, the only one that reads private repositories. Without it, the assistant only offers the token.
