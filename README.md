...
- **Try it without a GitHub account** — `GITFELLOW_FAKE_GITHUB=1 npm run dev` replaces GitHub with three repositories and eight weeks of made-up commits. The screenshots above come from it: `node scripts/pack.mjs && node scripts/screenshots.mjs`.
- **Enable “Sign in with GitHub”** — create an OAuth App on [github.com/settings/developers](https://github.com/settings/developers) with *Enable Device Flow* ticked (the callback URL is unused), then put its client ID in `src/lib/github-app.ts` (or `GITFELLOW_GITHUB_CLIENT_ID` at build time). Without it, the assistant only offers the token.
