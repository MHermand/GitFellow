...
- **Activer « Se connecter avec GitHub »** — créez une OAuth App sur [github.com/settings/developers](https://github.com/settings/developers) en cochant *Enable Device Flow* (l'URL de callback ne sert pas), puis mettez son client ID dans `src/lib/github-app.ts` (ou `GITFELLOW_GITHUB_CLIENT_ID` au moment du build). L'ID est public ; le device flow n'a pas de secret. L'app demande la portée `repo`, la seule qui lise les dépôts privés. Sans lui, l'assistant ne propose que le jeton.
