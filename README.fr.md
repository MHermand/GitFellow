...
- **Essayer sans compte GitHub** — `GITFELLOW_FAKE_GITHUB=1 npm run dev` remplace GitHub par trois dépôts et huit semaines de commits inventés. Les captures ci-dessus en viennent : `node scripts/pack.mjs && node scripts/screenshots.mjs`.
- **Activer « Se connecter avec GitHub »** — créez une OAuth App sur [github.com/settings/developers](https://github.com/settings/developers) en cochant *Enable Device Flow* (l'URL de callback ne sert pas), puis mettez son client ID dans `src/lib/github-app.ts` (ou `GITFELLOW_GITHUB_CLIENT_ID` au moment du build). Sans lui, l'assistant ne propose que le jeton.
