...
- **Essayer sans compte GitHub** — `GITFELLOW_FAKE_GITHUB=1 npm run dev` remplace GitHub par trois dépôts et huit semaines de commits inventés. Les captures ci-dessus en viennent : `npm i -D playwright && npx playwright install chromium` une fois, puis `node scripts/pack.mjs && node scripts/screenshots.mjs`.
- **Activer « Se connecter avec GitHub »** — créez une OAuth App sur [github.com/settings/developers](https://github.com/settings/developers) en cochant *Enable Device Flow* et en **décochant** *Expire user access tokens* (renouveler un jeton expiré exige un client secret, qu'une application installée sur les postes ne peut pas garder ; l'URL de callback ne sert pas), puis mettez son client ID dans `src/lib/github-app.ts` (ou passez `GITFELLOW_GITHUB_CLIENT_ID` au lancement, pour essayer sans toucher au fichier). L'ID est public ; le device flow n'a pas de secret. L'app demande la portée `repo`, la seule qui lise les dépôts privés. Sans lui, l'assistant ne propose que le jeton.
